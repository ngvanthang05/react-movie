...
const CategoryEdit = () => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            ...
            if (res.categorys) {
                const categoryFilter = res.categorys.filter((category) => category.id !== dataCategory.id)
                setDataCategories(categoryFilter)
            }
        }
        fetchCategories()
    }, [])
    const submitAction = async (event) => {
        ...
        const res = await create(`category/${dataCategory.id}`, formData)
        ...
        <div>
            ...
            <form onSubmit={submitAction} className="w-1/2 mx-auto">
                ...
                <div>
                    ...
                    <input id="name" name="name" type="text" placeholder="Name" defaultValue={dataCategory.name}
                ...
                <div className='mt-3'>
                    ...
                    <input id="description" name="description" type="text" placeholder="description" defaultValue={dataCategory.description}
                        className="block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                </div>

                <div className='mt-3'>
                    <label htmlFor="parent_id" className="block text-sm font-medium mb-2">Parent_id</label>
                    <select id="parent_id" name="parent_id" defaultValue={dataCategory.parent_id}
                        className="block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div className='mt-3'>
                    ...
                    <select id="sort_order" name="sort_order" defaultValue={dataCategory.sort_order}
                        className="block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div className='mt-3'>
                    ...
                    <select id="status" name="status" defaultValue={dataCategory.status}
                        className="block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        {statuses.map((status) => (
                ...
                <div className='mt-3'>
                    ...
                    <PickImage valueImageSrc={`/category/${dataCategory.image}`} />
                </div>
